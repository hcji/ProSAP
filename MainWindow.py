# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'MainWindow.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(1054, 874)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout.setObjectName("gridLayout")
        self.ButtonAddComplex = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonAddComplex.setObjectName("ButtonAddComplex")
        self.gridLayout.addWidget(self.ButtonAddComplex, 8, 0, 1, 1)
        self.ButtonRemoveComplex = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonRemoveComplex.setObjectName("ButtonRemoveComplex")
        self.gridLayout.addWidget(self.ButtonRemoveComplex, 8, 1, 1, 1)
        self.ButtonShowCurve = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonShowCurve.setObjectName("ButtonShowCurve")
        self.gridLayout.addWidget(self.ButtonShowCurve, 8, 2, 1, 1)
        self.tableProteinComplex = QtWidgets.QTableWidget(self.centralwidget)
        self.tableProteinComplex.setObjectName("tableProteinComplex")
        self.tableProteinComplex.setColumnCount(0)
        self.tableProteinComplex.setRowCount(0)
        self.gridLayout.addWidget(self.tableProteinComplex, 7, 0, 1, 3)
        self.labelProteinComplex = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelProteinComplex.setFont(font)
        self.labelProteinComplex.setObjectName("labelProteinComplex")
        self.gridLayout.addWidget(self.labelProteinComplex, 6, 0, 1, 2)
        self.ListFile = QtWidgets.QListWidget(self.centralwidget)
        self.ListFile.setObjectName("ListFile")
        self.gridLayout.addWidget(self.ListFile, 1, 0, 1, 3)
        self.ButtonGroup1 = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonGroup1.setObjectName("ButtonGroup1")
        self.gridLayout.addWidget(self.ButtonGroup1, 2, 0, 1, 1)
        self.tableProtein1 = QtWidgets.QTableView(self.centralwidget)
        self.tableProtein1.setObjectName("tableProtein1")
        self.gridLayout.addWidget(self.tableProtein1, 1, 3, 1, 1)
        self.labelDatabase = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelDatabase.setFont(font)
        self.labelDatabase.setObjectName("labelDatabase")
        self.gridLayout.addWidget(self.labelDatabase, 3, 0, 1, 2)
        self.labelThermalShift2 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelThermalShift2.setFont(font)
        self.labelThermalShift2.setObjectName("labelThermalShift2")
        self.gridLayout.addWidget(self.labelThermalShift2, 3, 4, 1, 1)
        self.tableProtein2 = QtWidgets.QTableView(self.centralwidget)
        self.tableProtein2.setObjectName("tableProtein2")
        self.gridLayout.addWidget(self.tableProtein2, 1, 4, 1, 1)
        self.labelThermalShift1 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelThermalShift1.setFont(font)
        self.labelThermalShift1.setObjectName("labelThermalShift1")
        self.gridLayout.addWidget(self.labelThermalShift1, 3, 3, 1, 1)
        self.labelFileList = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelFileList.setFont(font)
        self.labelFileList.setObjectName("labelFileList")
        self.gridLayout.addWidget(self.labelFileList, 0, 0, 1, 1)
        self.labelProteinTable1 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelProteinTable1.setFont(font)
        self.labelProteinTable1.setObjectName("labelProteinTable1")
        self.gridLayout.addWidget(self.labelProteinTable1, 0, 3, 1, 1)
        self.labelProteinTable2 = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        self.labelProteinTable2.setFont(font)
        self.labelProteinTable2.setObjectName("labelProteinTable2")
        self.gridLayout.addWidget(self.labelProteinTable2, 0, 4, 1, 1)
        self.ButtonGroup2 = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonGroup2.setObjectName("ButtonGroup2")
        self.gridLayout.addWidget(self.ButtonGroup2, 2, 1, 1, 1)
        self.ButtonClearFileList = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonClearFileList.setObjectName("ButtonClearFileList")
        self.gridLayout.addWidget(self.ButtonClearFileList, 2, 2, 1, 1)
        self.GraphicThermShift1 = QtWidgets.QGraphicsView(self.centralwidget)
        self.GraphicThermShift1.setObjectName("GraphicThermShift1")
        self.gridLayout.addWidget(self.GraphicThermShift1, 4, 3, 4, 1)
        self.GraphicThermShift2 = QtWidgets.QGraphicsView(self.centralwidget)
        self.GraphicThermShift2.setObjectName("GraphicThermShift2")
        self.gridLayout.addWidget(self.GraphicThermShift2, 4, 4, 4, 1)
        self.ListDatabase = QtWidgets.QListWidget(self.centralwidget)
        self.ListDatabase.setObjectName("ListDatabase")
        self.gridLayout.addWidget(self.ListDatabase, 4, 0, 1, 3)
        self.ButtonDatabaseRemove = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonDatabaseRemove.setObjectName("ButtonDatabaseRemove")
        self.gridLayout.addWidget(self.ButtonDatabaseRemove, 5, 1, 1, 1)
        self.ButtonClearDatabase = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonClearDatabase.setObjectName("ButtonClearDatabase")
        self.gridLayout.addWidget(self.ButtonClearDatabase, 5, 2, 1, 1)
        self.ButtonDatabaseConfirm = QtWidgets.QPushButton(self.centralwidget)
        self.ButtonDatabaseConfirm.setObjectName("ButtonDatabaseConfirm")
        self.gridLayout.addWidget(self.ButtonDatabaseConfirm, 5, 0, 1, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 1054, 26))
        self.menubar.setObjectName("menubar")
        self.menuFile = QtWidgets.QMenu(self.menubar)
        self.menuFile.setObjectName("menuFile")
        self.menuDatabase = QtWidgets.QMenu(self.menubar)
        self.menuDatabase.setObjectName("menuDatabase")
        self.menuHelp = QtWidgets.QMenu(self.menubar)
        self.menuHelp.setObjectName("menuHelp")
        self.menuAnalysis = QtWidgets.QMenu(self.menubar)
        self.menuAnalysis.setObjectName("menuAnalysis")
        MainWindow.setMenuBar(self.menubar)
        self.actionOpen = QtWidgets.QAction(MainWindow)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("img/open.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionOpen.setIcon(icon)
        self.actionOpen.setObjectName("actionOpen")
        self.actionSave = QtWidgets.QAction(MainWindow)
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("img/save.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionSave.setIcon(icon1)
        self.actionSave.setObjectName("actionSave")
        self.actionQuit = QtWidgets.QAction(MainWindow)
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("img/quit.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionQuit.setIcon(icon2)
        self.actionQuit.setObjectName("actionQuit")
        self.actionProteomics = QtWidgets.QAction(MainWindow)
        self.actionProteomics.setObjectName("actionProteomics")
        self.actionDatabase = QtWidgets.QAction(MainWindow)
        self.actionDatabase.setObjectName("actionDatabase")
        self.action_CETSA = QtWidgets.QAction(MainWindow)
        self.action_CETSA.setObjectName("action_CETSA")
        self.actionCalcPval = QtWidgets.QAction(MainWindow)
        self.actionCalcPval.setObjectName("actionCalcPval")
        self.actionCalcROC = QtWidgets.QAction(MainWindow)
        self.actionCalcROC.setObjectName("actionCalcROC")
        self.actionCheck_update = QtWidgets.QAction(MainWindow)
        self.actionCheck_update.setObjectName("actionCheck_update")
        self.actionCitation = QtWidgets.QAction(MainWindow)
        self.actionCitation.setObjectName("actionCitation")
        self.actionCheck_liences = QtWidgets.QAction(MainWindow)
        self.actionCheck_liences.setObjectName("actionCheck_liences")
        self.actionPreprocessing = QtWidgets.QAction(MainWindow)
        self.actionPreprocessing.setObjectName("actionPreprocessing")
        self.menuFile.addAction(self.actionOpen)
        self.menuFile.addAction(self.actionSave)
        self.menuFile.addAction(self.actionQuit)
        self.menuDatabase.addAction(self.actionPreprocessing)
        self.menuDatabase.addAction(self.actionProteomics)
        self.menuDatabase.addAction(self.actionDatabase)
        self.menuHelp.addAction(self.actionCheck_update)
        self.menuHelp.addAction(self.actionCitation)
        self.menuHelp.addAction(self.actionCheck_liences)
        self.menuAnalysis.addAction(self.action_CETSA)
        self.menuAnalysis.addAction(self.actionCalcPval)
        self.menuAnalysis.addAction(self.actionCalcROC)
        self.menubar.addAction(self.menuFile.menuAction())
        self.menubar.addAction(self.menuDatabase.menuAction())
        self.menubar.addAction(self.menuAnalysis.menuAction())
        self.menubar.addAction(self.menuHelp.menuAction())

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.ButtonAddComplex.setText(_translate("MainWindow", "Add"))
        self.ButtonRemoveComplex.setText(_translate("MainWindow", "Remove"))
        self.ButtonShowCurve.setText(_translate("MainWindow", "Show Curve"))
        self.labelProteinComplex.setText(_translate("MainWindow", "Protein Complex"))
        self.ButtonGroup1.setText(_translate("MainWindow", "Set as G1"))
        self.labelDatabase.setText(_translate("MainWindow", "Database List"))
        self.labelThermalShift2.setText(_translate("MainWindow", "Group2 Thermal Shift Curve"))
        self.labelThermalShift1.setText(_translate("MainWindow", "Group1 Thermal Shift Curve"))
        self.labelFileList.setText(_translate("MainWindow", "File List"))
        self.labelProteinTable1.setText(_translate("MainWindow", "Group1 Protein Table"))
        self.labelProteinTable2.setText(_translate("MainWindow", "Group2 Protein Table"))
        self.ButtonGroup2.setText(_translate("MainWindow", "Set as G2"))
        self.ButtonClearFileList.setText(_translate("MainWindow", "Clear"))
        self.ButtonDatabaseRemove.setText(_translate("MainWindow", "Remove"))
        self.ButtonClearDatabase.setText(_translate("MainWindow", "Clear"))
        self.ButtonDatabaseConfirm.setText(_translate("MainWindow", "Confirm"))
        self.menuFile.setTitle(_translate("MainWindow", "File"))
        self.menuDatabase.setTitle(_translate("MainWindow", "Data"))
        self.menuHelp.setTitle(_translate("MainWindow", "Help"))
        self.menuAnalysis.setTitle(_translate("MainWindow", "Analysis"))
        self.actionOpen.setText(_translate("MainWindow", "Open..."))
        self.actionOpen.setShortcut(_translate("MainWindow", "Ctrl+O"))
        self.actionSave.setText(_translate("MainWindow", "Save..."))
        self.actionSave.setShortcut(_translate("MainWindow", "Ctrl+S"))
        self.actionQuit.setText(_translate("MainWindow", "Quit"))
        self.actionQuit.setShortcut(_translate("MainWindow", "Ctrl+Q"))
        self.actionProteomics.setText(_translate("MainWindow", "Load proteomics..."))
        self.actionDatabase.setText(_translate("MainWindow", "Load PPI database..."))
        self.action_CETSA.setText(_translate("MainWindow", "Single protein analysis (TSA) ..."))
        self.actionCalcPval.setText(_translate("MainWindow", "Calculate p-values of protein complex ..."))
        self.actionCalcROC.setText(_translate("MainWindow", "Calculate ROC for protein complex discrimination ..."))
        self.actionCheck_update.setText(_translate("MainWindow", "Check update ..."))
        self.actionCitation.setText(_translate("MainWindow", "Check citation ..."))
        self.actionCheck_liences.setText(_translate("MainWindow", "Check licence"))
        self.actionPreprocessing.setText(_translate("MainWindow", "Preprocessing..."))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
